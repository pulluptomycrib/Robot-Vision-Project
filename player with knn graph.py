# p2.py - SimpleNavPlayer with Goal ID, Visual Feedback, and Graph-based Navigation
# Provides navigation guidance using VLAD distance and a graph of image IDs
...
class SimpleNavPlayer(Player):
    def __init__(self):
        ...
        # Debug: Print Action enum values
        print("Action enum values:")
        for action in dir(Action):
            if not action.startswith('__'):
                print(f"Action.{action} = {getattr(Action, action)}")

    def build_knn_graph(self):
        """Build a k-NN graph of image IDs based on VLAD distances and save it as knn_graph.pkl."""
        ...
        graph = nx.Graph()
        # Add nodes (image IDs from 0 to 14999)
        for i in range(len(self.vlad_database)):
            graph.add_node(i)

        # Compute distances and add edges to the top k nearest neighbors
        k = 10  # Number of nearest neighbors
        lambda_x = 1.0  # Weight for sorting (can adjust if needed)
        for i in tqdm(range(len(self.vlad_database)), desc="Building graph"):
            distances = [
                (j, np.linalg.norm(self.vlad_database[i] - self.vlad_database[j]))
                for j in range(len(self.vlad_database)) if j != i
            ]
    ...
    def act(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                print(f"[KeyDown] Key pressed: {event.key}")
                if event.key in self.keymap:
                    self.last_act |= self.keymap[event.key]
                    print(f"[Action] Applied action: {self.last_act}")
                elif event.key == pygame.K_q:
                    return self.navigate_to_goal()  # Return the action directly
            if event.type == pygame.KEYUP:
                if event.key in self.keymap:
                    self.last_act ^= self.keymap[event.key]
                    print(f"[Action] Released action: {self.last_act}")

        # Ensure the Pygame window has focus
        if self.screen and not pygame.display.get_active():
            pygame.display.set_caption("SimpleNavPlayer:fpv - Click to Focus")
            pygame.event.pump()

        # Debug: Force a movement action to test if the game responds
        # Uncomment the following line to test if any movement works
        # self.last_act = Action.FORWARD
        # print("[Debug] Forcing Action.FORWARD")

        print(f"[Act] Returning action: {self.last_act}")
    ...
    def navigate_to_goal(self):
        """Estimate proximity to goal, find the shortest path, suggest an action, and display images."""
        if self.fpv is None:
        ...
        if closest_idx is not None:
            print(f"[Navigate] You're at ID {closest_idx}. Closest goal ID is {self.goal_id}.")

        # Find the shortest path from the current image ID to the goal image ID
        try:
            path = nx.astar_path(self.graph, closest_idx, self.goal_id, weight='weight')
            path_str = " -> ".join(map(str, path))
            print(f"[Navigate] Shortest path: {path_str}")
        except nx.NetworkXNoPath:
            print("[Navigate] No path found to the goal. Trying a default action sequence.")
            path = [closest_idx]

        # Determine the next image ID to move toward
        next_idx = path[1] if len(path) > 1 else closest_idx
        print(f"[Navigate] Next step: Move from ID {closest_idx} to ID {next_idx}.")
        ...
        # Suggest an action based on the next step in the path
        # For simplicity, we'll cycle through the action sequence
        # In a real implementation, you'd use spatial reasoning to determine the correct action
        ...
        print("[Navigate] After moving, press Q again to check if you're closer to the goal.")
