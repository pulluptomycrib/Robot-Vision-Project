...
class KeyboardPlayerPyGame(Player):
    ...
    def pre_navigation(self):
        ...
        # Load the four target views and find the closest match for each
        print("Finding closest matches for target views...")
        target_view_names = ["front", "back", "right", "left"]
        exploration_observation = natsorted([x for x in os.listdir(self.save_dir) if x.endswith('.jpg')])
        
        for view_name in target_view_names:
            view_path = f"target_{view_name}.jpg"
            if not os.path.exists(view_path):
                print(f"Target view {view_path} not found in the project directory.")
                continue
            view_img = cv2.imread(view_path)
            if view_img is None:
                print(f"Failed to load {view_path}.")
                continue
            
            # Compute VLAD descriptor for the target view
            view_vlad = self.get_VLAD(view_img).reshape(1, -1)
            
            # Find the closest match using BallTree
            _, index = self.tree.query(view_vlad, 1)
            closest_id = index[0][0]
            closest_image_name = exploration_observation[closest_id]
            
            print(f"Closest match for target_{view_name}.jpg: ID {closest_id} (image: {closest_image_name})")
